            ...
            for x in range(self.w_idx):
                ...
                if self.opacity_grid[y][x][1] == -1:  # More Black
                    ...
                    if self.opacity_grid[y][x][0] == 0:
                        ...
                        self.opacity_grid[y][x][2] = [0, 0, 0]

                    color = (self.opacity_grid[y][x][0] * self.opacity_grid[y][x][2][0],
                             self.opacity_grid[y][x][0] * self.opacity_grid[y][x][2][1],
                             self.opacity_grid[y][x][0] * self.opacity_grid[y][x][2][0])
                    ...
                    if self.opacity_grid[y][x][0] == 1:
                        self.opacity_grid[y][x][1] = 0
                        self.opacity_grid[y][x][2] = [255,255,255]

                    color = (self.opacity_grid[y][x][2][0] + self.opacity_grid[y][x][0] * (255 - self.opacity_grid[y][x][2][0]),
                             self.opacity_grid[y][x][2][1] + self.opacity_grid[y][x][0] * (255 - self.opacity_grid[y][x][2][1]),
                             self.opacity_grid[y][x][2][2] + self.opacity_grid[y][x][0] * (255 - self.opacity_grid[y][x][2][2]))
...
while True:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            try:
                if game.grid[y][x]:
                    ...
                    score += 1
                    temp_score += 1
                    game.opacity_grid[y][x][2] = [	166, 255, 77]
    ...
    if temp_score % 40 == 0 and temp_score > 0:
        temp_score = 0
        time += 10
    ...
    score_txt = font.render(str(score), False, (0, 0, 0))
    ...
    if time == -1:  # Further investigation needed on why it has to be -1 and not 0
        print("reset", score)
        exec(open("Rough Draft 6.py").read())
